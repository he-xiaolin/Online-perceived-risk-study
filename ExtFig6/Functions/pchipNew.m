...
%         yq = ppval(yq, xq);
% 
%         % Set the value for points outside the last data point to the last value
%         yq(xq > x(end)) = y(end);
%     end
% end
...
%         yq = ppval(yq, xq);
% 
%         % Apply constraints for values outside the original data range
%         yq(xq < x(1)) = max(0, min(10, y(1) + (xq(xq < x(1)) - x(1)) * slopes(1)));
%         yq(xq > x(end)) = max(0, min(10, y(end) + (xq(xq > x(end)) - x(end)) * slopes(end)));
%     end
% end
% 
% function d = modifiedPchipSlopes(h, del)
...
%     d = zeros(1, n); % Initialize all slopes to zero
% 
%     % Set the first and last slopes based on the input data
%     d(1) = del(1); % Slope at the first point
%     d(n) = del(end); % Slope at the last point
% 
%     % Compute slopes for the remaining points
%     for j = 2:n-1
%         % Check if the signs of successive slopes (delta values) are the same
