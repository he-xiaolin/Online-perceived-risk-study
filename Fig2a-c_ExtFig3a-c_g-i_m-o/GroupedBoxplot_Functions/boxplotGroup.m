function pValues = boxplotGroup(varargin)
...
%%
% Perform ANOVA for each group and mark significant differences
grouped_data=cell(1,1);
for i = 1:nGroups
    % Extract data for current group
    groupData = cellfun(@(x) x(:,i), p.Results.x, 'UniformOutput', false);
    grouped_data{i}= groupData;
end

% Get the number of arrays
numArrays1 = length(grouped_data);
numArrays2 = length(grouped_data{1});
% Initialize matrix to store p-values
pValues = cell(1,numArrays1);%zeros(numArrays1);
for i = 1:length(pValues)
    pValues{i} = zeros(numArrays2,numArrays2); % Or whatever values you want to initialize to
end

for k=1:numArrays1
% Loop over all pairs of arrays
for i = 1:numArrays2
    for j = i+1:numArrays2
        % Perform two-sample t-test
        [~,pValues{k}(i,j)] = ttest2(grouped_data{1,k}{1,i}, grouped_data{1,k}{1,j});
    end
end
end
...
for i = 1:numArrays2
    for j = i+1:numArrays2
        xpos=[i+(k-1).*(numArrays2+1)  j+(k-1).*(numArrays2+1)];
        ypos=[upperLevel upperLevel];
        if  pValues{k}(i,j)<=0.001
            line(xpos,ypos); 
            plot((xpos(1)+xpos(2))/2-0.25, upperLevel+0.1,'k*');hold on
            plot((xpos(1)+xpos(2))/2, upperLevel+0.1,'k*');
            plot((xpos(1)+xpos(2))/2+0.25, upperLevel+0.1,'k*');
            upperLevel=upperLevel+0.2;
        elseif pValues{k}(i,j)<=0.01
            line(xpos,ypos); 
            plot((xpos(1)+xpos(2))/2-0.25, upperLevel+0.1,'k*');hold on
            plot((xpos(1)+xpos(2))/2, upperLevel+0.1,'k*');
            upperLevel=upperLevel+0.2;
        elseif pValues{k}(i,j)<=0.05
            line(xpos,ypos); hold on;
            plot((xpos(1)+xpos(2))/2, upperLevel+0.1,'k*');
            upperLevel=upperLevel+0.2;
        else
            continue;
...
if ~isempty(p.Results.secondarylabels)
    if any(strcmpi(groupLabelType, {'horizontal','both'}))
        ...
        if isUIFigure
            ...
            h.axis2 = axFcn(h.figure,'Units',h.axis.Units, 'OuterPosition', h.axis.OuterPosition, ...
                ...
                'XTickLabelRotation',0,'box','off','FontSize',20,...%h.axis.FontSize,...
    ...
    if any(strcmpi(groupLabelType, {'vertical','both'})) && ~verLessThan('Matlab','9.5') % r18b
        ...
        h.xline = arrayfun(@(x)xline(h.axis, x,'FontSize',20,...%h.axis.FontSize,...
