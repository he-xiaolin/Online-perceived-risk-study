...
mkdir('processed_feature_v5/MB/regression_input');
mkdir('processed_feature_v5/MB/regression_label');
data_ = load(pt);
data = data_.MB;
for i=1:27 %   MB event order 1-27

Time = data(i).Time;
% x_s = data(i).x_s; % eliminated because of the predictable pattern over
% time.
...
% y_s = data(i).y_s; % eliminated because of the predictable pattern over
% time
% y_n = data(i).y_n; % eliminated because of the predictable pattern over
% time
...
File_name =append('processed_feature_v5/MB/regression_input/MB_input_',num2str(i,'%02d'),'.mat');
save(File_name,'Time', ...
        'vx_s', 'vx_n', ...
        'ax_s' ,'ax_n' ,...
        'vy_s' ,'vy_n' , ...
...
for i = 1:27
    ...
    File_name =append('processed_feature_v5/MB/regression_label/MB_rating_',num2str(i,'%02d'),'.mat');
