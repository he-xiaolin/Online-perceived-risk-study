...
%%
clear;clc;close all;
pt = '../0-Raw_Data/KineticData/HB/HB_';
mkdir('processed_feature');

for nn=1:27 %   hb event order 1-27
load([pt, num2str(nn),'.mat'])
    % Define all kinetic data. Traffic_T18 is the leading vehicle on the
    % main road
    x_s = DATASTRUCTURE.Car_Con_tx.data;
    x_n = DATASTRUCTURE.Traffic_T18_tx.data;
    vx_s = DATASTRUCTURE.Car_Con_vx_1.data;
    vx_n = DATASTRUCTURE.Traffic_T18_v_0_x.data;
    ax_s = DATASTRUCTURE.Car_Con_ax.data;
    ax_n = DATASTRUCTURE.Traffic_T18_a_1_x.data;
    ax_n(abs(ax_n)>50)=0;
    vx_n(abs(vx_n)>50)=0;
    y_s = DATASTRUCTURE.Car_Con_ty.data;
    y_n = DATASTRUCTURE.Traffic_T18_ty.data;
    vy_s = DATASTRUCTURE.Car_Con_vy_1.data;
    vy_n = DATASTRUCTURE.Traffic_T18_v_0_y.data;
    ay_s = DATASTRUCTURE.Car_Con_ay.data;
    ay_n = DATASTRUCTURE.Traffic_T18_a_1_y.data;
    ay_n(abs(ay_n)>50)=0;
    vy_n(abs(vy_n)>50)=0;
    delta_x=x_n-x_s;
    delta_y=abs(y_n-y_s);
    ...
    len = length(Duration);
    Time=DATASTRUCTURE.Time.data;
    File_name =append('processed_feature/HB/regression_input/HB_input_',num2str(nn,'%02d'),'.mat');
