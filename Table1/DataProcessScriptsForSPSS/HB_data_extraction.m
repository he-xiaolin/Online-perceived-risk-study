...
braking_distance_values = [5, 15, 25, 5, 15, 25, 5, 15, 25, 5, 15, 25, 5, 15, 25, 5, 15, 25, 5, 15, 25, 5, 15, 25, 5, 15, 25];

% Iterate over the 27 event numbers
for i = 1:27
    % Extract event ratings and participant IDs for this event number
    event_ratings_condition = HB_PR(i).event_ratings; 
    idx_condition = HB_PR(i).idx;
    ...
    for clip = 1:5
        ...
        braking_distance = [braking_distance; repmat(braking_distance_values(i), size(idx_condition))];
    end
end

% Create a table to store the reorganized data
data_table = table(event_numbers, participant_ids, clip_numbers, ...
                   speed, braking_intensity, braking_distance, perceived_risk_ratings, ...
                   'VariableNames', {'EventNumber', 'ParticipantID', 'ClipNumber', ...
                                     'Speed', 'BrakingIntensity', 'BrakingDistance', 'PerceivedRiskRating'});

% Save the table as a CSV file for import into SPSS
writetable(data_table, 'reorganized_HB_data.csv');
